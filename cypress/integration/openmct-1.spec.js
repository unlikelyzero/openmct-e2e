...
describe('OPENMCT-1', () => {
    ...
    it('Application does not render table if telemetry data is not received from server', () => {
        cy.intercept('/sockjs-node/info?t=**', {forceNetworkError: true})
    ...
    it('Application recieves 15 minutes of historical telemetry data from server when choosing each or all telemetry point(s)', () => {
        cy.intercept('GET', '/sockjs-node/info?t=**').as('loaded')
        cy.visit('/')
        ...
        cy.checkTableFirstAndLastValue(15)
        cy.get('[value="pwr.v"]').click({shiftKey: true})
        cy.checkTableFirstAndLastValue(15)
    })
    it('Application displays locale-based timestamp format for non-en browsers', () => {
